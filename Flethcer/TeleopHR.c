...
void operate_hand_vertical() {
	if (joystick.joy2_TopHat == 0) // up on D-pad
		servo[hand_vertical] = 157; //tune it

	if (joystick.joy2_TopHat == 4) // down on D-pad
		servo[hand_vertical] = 50; //tune it
}
...
/*void test(){
	if (joy1Btn(2) && nMotorEncoder[ScissorL1] != 0)	{   //The A Button
